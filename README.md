...
7. Equality can be handled for series of different types - e.g. ```[1.0]``` could be equated to ```[True]``` and the result could be [False] but since it was told that we have fail fast on the type of list, we throw an exception for the equality also. This feature has been implemented but disabled. To enable the feature see [series.py](/src/pandas_exp/series.py) *__eq__* function and [test_series.py](/test/pandas_exp/test_series.py) *test_valid_equality_on_series* and *test_invalid_equality_on_series* function.
